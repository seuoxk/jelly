...
export default function MainScreen({ navigation }) {
  ...
  useEffect(() => { AsyncStorage.getItem('favoriteBeaches').then((value) => setFavorites(JSON.parse(value || '[]'))); }, []);
  ...
  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>BEACH SAFETY</Text><Text style={styles.title}>안 쏘고 뭐海</Text><Text style={styles.brandSubtitle}>{isGuest ? '게스트 님 환영합니다!' : `${user.nickname} 님, 환영합니다!`}</Text></View>{isGuest ? <TouchableOpacity style={styles.loginHeader} onPress={() => navigation.getParent()?.navigate('Auth')}><Text style={styles.loginHeaderText}>로그인</Text></TouchableOpacity> : <View style={styles.waveBadge}><Ionicons name="water" size={24} color="#0369A1" /></View>}</View>
    {isGuest && <TouchableOpacity style={styles.guestBanner} onPress={() => navigation.getParent()?.navigate('Auth')}><Ionicons name="cloud-upload-outline" size={18} color="#075985" /><Text style={styles.guestBannerText}>로그인 시 스캔 기록 및 즐겨찾기 동기화 가능</Text><Text style={styles.guestLogin}>로그인</Text></TouchableOpacity>}
    <FavoritesBar favorites={favorites} onToggle={async (name) => { const next = favorites.includes(name) ? favorites.filter((item) => item !== name) : [...favorites, name]; setFavorites(next); await AsyncStorage.setItem('favoriteBeaches', JSON.stringify(next)); }} />
    <NearbyBeachCard nearest={nearest} nearbyBeaches={nearbyBeaches} state={locationState} message={locationMessage} timeText={timeText} isOpenHours={isOpenHours} inSeason={inSeason} onRetry={findLocation} favorites={favorites} onToggle={async (name) => { const next = favorites.includes(name) ? favorites.filter((item) => item !== name) : [...favorites, name]; setFavorites(next); await AsyncStorage.setItem('favoriteBeaches', JSON.stringify(next)); }} />
...
function FavoritesBar({ favorites, onToggle }) { return <View style={styles.favorites}><Text style={styles.favoritesTitle}>관심 해수욕장</Text><View style={styles.favoriteRow}>{['해운대 해수욕장', '광안리 해수욕장', '경포 해수욕장'].map((name) => <TouchableOpacity key={name} onPress={() => onToggle(name)} style={[styles.favoriteChip, favorites.includes(name) && styles.favoriteActive]}><Ionicons name={favorites.includes(name) ? 'heart' : 'heart-outline'} size={13} color={favorites.includes(name) ? '#FF4D4D' : '#64748B'} /><Text style={styles.favoriteText}>{name.replace(' 해수욕장', '')}</Text></TouchableOpacity>)}</View></View>; }
function NearbyBeachCard({ nearest, nearbyBeaches, state, message, timeText, isOpenHours, inSeason, onRetry, favorites, onToggle }) {
...
Object.assign(styles, StyleSheet.create({ favorites: { marginBottom: 12 }, favoritesTitle: { color: '#0A192F', fontWeight: '800', fontSize: 14, marginBottom: 7 }, favoriteRow: { flexDirection: 'row', gap: 7, flexWrap: 'wrap' }, favoriteChip: { flexDirection: 'row', gap: 4, alignItems: 'center', backgroundColor: '#FFF', paddingHorizontal: 9, paddingVertical: 6, borderRadius: 13 }, favoriteActive: { backgroundColor: '#FFF0F0' }, favoriteText: { color: '#475569', fontSize: 12, fontWeight: '700' } }));
Object.assign(styles, StyleSheet.create({ loginHeader: { backgroundColor: '#00B4D8', borderRadius: 14, paddingHorizontal: 12, paddingVertical: 8 }, loginHeaderText: { color: '#0A192F', fontWeight: '900', fontSize: 12 }, guestBanner: { backgroundColor: '#DDF6FB', borderRadius: 13, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 14 }, guestBannerText: { color: '#075985', fontSize: 12, flex: 1, fontWeight: '700' }, guestLogin: { color: '#0369A1', fontWeight: '900', fontSize: 12 } }));
