...
export default function IntroScreen({ navigation }) {
  ...
  return <SafeAreaView style={s.safe}><Animated.View style={[s.container, { opacity: fade }]}><View style={s.hero}><Ionicons name="water" size={68} color="#00B4D8" /><Text style={s.brand}>sea fari</Text><Text style={s.tagline}>더 안전하게, 바다를 즐기세요</Text></View><View>{features.map(([icon, label]) => <View style={s.feature} key={label}><Ionicons name={icon} size={25} color="#00B4D8" /><Text style={s.featureText}>{label}</Text></View>)}</View><TouchableOpacity style={s.guest} onPress={() => navigation.replace('AppTabs')}><Text style={s.guestText}>시작하기</Text></TouchableOpacity></Animated.View></SafeAreaView>;
}
